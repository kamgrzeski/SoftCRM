<!DOCTYPE html>
<html lang="pl">
@include('layouts.head', ['title' => 'Update task'])
        ...
        <main class="flex-1 p-6 overflow-y-auto">
            ...
            <div class="w-full bg-white shadow-md rounded-lg mb-3">
                <div class="p-6 flex justify-between items-center">
                    <p class="text-xl">Update task: {{ $task->name }}</p>
                    <a href="{{ url()->previous() }}">
                ...
                <div class="p-6">
                    <form action="{{ route('tasks.update', $task) }}" method="POST">
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            ...
                            <div>
                                <div class="mb-4">
                                    <label for="employee_id" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Assign employee</label>
                                    <div class="flex">
                                        ...
                                        <select id="employee_id" name="employee_id" required
                                            ...
                                            @foreach($employees as $key => $employee)
                                                <option value="{{ $employee->id }}" @if($task->employee_id == $employee->id) selected @endif>{{ $employee->full_name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div class="flex justify-end border-t border-gray-200">
                            <button type="submit" class="bg-blue-500 mt-3 text-white font-semibold py-2 px-4 rounded-md hover:bg-blue-600 focus:outline-none focus:bg-blue-600">Update test</button>
                        </div>
                    </form>
                </div>
            </div>
