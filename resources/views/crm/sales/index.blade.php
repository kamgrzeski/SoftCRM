                            ...
                            <tr class="bg-white hover:bg-gray-50">
                                ...
                                <td class="px-4 py-2 text-center border border-gray-300">
                                    <button type="submit" class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold py-1 px-4 border border-gray-400 rounded shadow">
                                        {{ Cknow\Money\Money::{\App\Queries\SettingsQueries::getSettingValue('currency')}
                                ...
                                <td class="px-4 py-2 text-center border border-gray-300">
                                    <form method="POST" action="{{ route('sales.set.active', $sale) }}">
                                        ...
                                        <label class="inline-flex items-center cursor-pointer">
                                            <input type="checkbox" value="" class="sr-only peer"  onchange="this.form.submit()" @if($sale->is_active) checked @endif>
                                ...
                                <td class="px-4 py-2 text-center border border-gray-300">
                                    <div class="flex justify-center items-center space-x-2">
                                        <a href="{{ route('sales.view', $sale) }}" class="bg-blue-500 text-white active:bg-blue-600 font-bold uppercase text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none ease-linear transition-all duration-150">
                                            More info
                                        </a>

                                        <a href="{{ route('sales.update.form', $sale) }}" class="bg-yellow-500 text-white active:bg-blue-600 font-bold uppercase text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none ease-linear transition-all duration-150">
                                            Edit
                                        </a>
